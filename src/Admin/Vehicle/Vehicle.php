...
class Vehicle extends Connection{
	...
	public function image_upload($file){
		...
		move_uploaded_file($file['tmp_name'], '../uploads/vehicle/'.$image);
		return $image;
	}
	public function insert(){
			
		if(empty($this->image)){
			$query = $this->con->prepare("insert into vehicle (vehicle,unique_id,created_at) values(:vehicle,:unique_id,:created_at)");
			...
			$query = $this->con->prepare("insert into vehicle (vehicle,image,unique_id,created_at) values(:vehicle,:image,:unique_id,:created_at)");
			$query->bindValue(':vehicle', $this->vehicle, PDO::PARAM_STR);
	...
	public function view(){
		$query = $this->con->prepare("select * from vehicle where status=:status order by id desc");
		$query->bindValue(':status', 1, PDO::PARAM_STR);
		$query->execute();
		return $query->fetchAll(PDO::FETCH_ASSOC);	
	}
	public function details($id){
		$query = $this->con->prepare("select * from vehicle where status=:status and unique_id=:id");
	...
	public function update_image($id,$file){
		...
		$image = rand(10000,90000).$file['name'];
		$moving = '../uploads/vehicle/'.$image;
		
		if(empty($data['image'])){
			move_uploaded_file($file['tmp_name'], $moving);
		}
		else if(file_exists('../uploads/vehicle/'.$data['image'])){
	...
	public function update(){
		...
		if(empty($this->image)){
			$query = $this->con->prepare("update vehicle set vehicle=:vehicle,updated_at=:updated_at where status=:status and unique_id=:id");
			...
			$query = $this->con->prepare("update vehicle set vehicle=:vehicle,image=:image,updated_at=:updated_at where status=:status and unique_id=:id");
	...
	public function delete($id){
		$query = $this->con->prepare("update vehicle set deleted_at=:deleted_at,status=:status where unique_id=:id");
		$query->bindValue(':id',$this->id, PDO::PARAM_INT);
