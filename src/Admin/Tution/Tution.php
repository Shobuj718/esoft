...
class Tution extends Connection{
	...
	public function insert(){
		...
		$query = $this->con->prepare("insert into achievement(achievement,status,unique_id,created_at) values(:achievement,:status,:unique_id,:created_at)");
		...
		if($query){
			...
			echo "<script>window.open('tution_fees.php','_self')</script>";
		}	
	}
	...
	public function update(){
		...
		if($query){
			...
			echo "<script>window.location='tution_fees.php'</script>";
		}	
	}
