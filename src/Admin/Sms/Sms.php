...
//            $stmt->bindValue(':insertDT', $insertDate, PDO::PARAM_STR);
	...
	//update attend data in student_data table for showing attend status
	public function updateStudent_data_table_attend_field($class, $section, $student_id){
		try {

		    $stmt = $this->con->prepare("update student_data set attend = :attend where student_id = :id  and class=:class and branch=:branch and status = :status");
		    $stmt->bindValue(':attend', present, PDO::PARAM_STR);
